...
function prepareDistFolder(){
    ...
    var commands =  [ 'git stash', 'git checkout ' + process.env.MASTERPACK_BRANCH, 'git pull' ];
    var settings = {cwd: dist};
    var execResult = null;
    initDist(dist).then(execCommands.bind(this,commands, settings)).catch(function(failed){
        var error = failed.error;
        //If failed due to branch not existing, create it.
        if(error.message && error.message.indexOf("'" + process.env.MASTERPACK_BRANCH + "' did not match any") >= 0 ){
            execCommands(['git checkout --orphan ' + process.env.MASTERPACK_BRANCH], settings).then(function(){
...
function initDist(folder){
    ...
    fs.emptyDir(folder, function(err){
        if(err)
            ...
            fs.copy(path.join(process.env.MASTERPACK_SOURCE, ".git"), path.join(folder, ".git"), function (err) {
                if (err)
                    deferred.reject(err);
                else
                    deferred.resolve(true)
...
function gitAddCommit(dirDetails){
    var deferred = q.defer();
    //Skip if MASTERPACK_SKIPCOMMIT is set
    if (parseInt(process.env.MASTERPACK_SKIPCOMMIT) > 0){
        console.log('Commit skipped, per value of ENV MASTERPACK_SKIPCOMMIT');
        return q(true);
    }

    var commands = ['git add .', 'git commit --message="Build Master:' + new Date().toISOString() + '"', 'git push'];
    var settings = {cwd: path.join(process.env.MASTERPACK_SOURCE, process.env.MASTERPACK_OUTPUT_FOLDER) };

    execCommands(commands, settings).then(function(success){
       deferred.resolve(true);
    }, function(failed){
        var error = failed.error;
        if(error.message && error.message.indexOf("The current branch "+ process.env.MASTERPACK_BRANCH +" has no upstream branch.") >= 0){
            execCommand('git push --set-upstream origin ' + process.env.MASTERPACK_BRANCH, settings).then(function(){
                deferred.resolve(true);
            }, function(failure){
