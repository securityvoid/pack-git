...
module.exports.createDistribution = function(){
    require('dotenv').config({path: path.join(process.env.DEPLOYMENT_SOURCE, '.masterpack') });
    checkEnvVariables();
...
function prepareDistFolder(){
    var deferred = q.defer();
    var dist = path.join(process.env.DEPLOYMENT_SOURCE, process.env.DEPLOY_DIST_FOLDER);
    fs.emptyDirSync(dist);
    fs.ensureDir(dist, function (err) {
        if(err){
            deferred.reject({success : false, error : err});
        } else {
            fs.copy(path.join(process.env.DEPLOYMENT_SOURCE, ".git"), path.join(dist, ".git"), function (err) {
                if (err)
                    deferred.reject({success : false, error : err});
                else {
                    exec('git stash', {cwd: dist}, function(error, stdout, stderr) {
                        if (error) {
                            ...
                            exec('git checkout master', {cwd: dist}, function (error1, stdout1, stderr1) {
                                if (error) {
...
function webPackIt(webPackLib){
    ...
    const toIgnore = JSON.parse(process.env.IGNORED_MODULES);
    var excludes = {};
    for (var x = 0; x < toIgnore.length; x++){
        excludes[toIgnore[x].toLowerCase()] = toIgnore[x];
    }
...
function checkEnvVariables(){
    ...
    try{
        var t = JSON.parse(process.env.IGNORED_MODULES);
        if(t.length < 1)
            throw new Error("Default Not Set");
    } catch(exception){
        console.log("WARNING: IGNORED_MODULES is not set, or not set to an array. Values defaulted.");
        console.log('Enter a proper value in .masterpack or as an ENV variable. e.g. ["crypto"]');
        process.env.IGNORED_MODULES = '["crypto"]';
    }
}
...
function updateIndex(pathResults){
    ...
    fs.readFile(index, 'utf8', function (err,data) {
        ...
        var requireFile = "require('" + path.join('..', process.env.WEBPACK_OUTPUT_FILE).replace(/[\\]/g, '\\$&') + "');\n";
