...
function prepareDistFolder(){
    ...
    var distResult = initDist(dist);
    var commands =  [ 'git stash', 'git checkout ' + process.env.MASTERPACK_BRANCH ];
    var settings = {cwd: dist};
    var execCmdsResult = distResult.then(function(){
        return execCommands(commands, settings);
    });

    var final = execCmdsResult.then(function(){
        return execCommand('git pull', settings);
    }, function(error){
        if(error.message && error.message.indexOf("'" + process.env.MASTERPACK_BRANCH + "' did not match any") >= 0 ){
            return execCommand('git checkout --orphan ' + process.env.MASTERPACK_BRANCH, settings);
        } else {
            return q.reject(error);
        }
    })

    return q.all([distResult, execCmdsResult, final]);
}
 ...
 * @returns {promise} - Resolve - { success: true, stdout, sderr, error}
 *                      Reject  - { success: false, stdout, sderr, error}
 */
function execCommands(commands, settings){
    var results = [];
    if(typeof commands === 'string')
        results.push(execCommand(commands, settings));
    else {
        for(var x =0; x < commands.length; x++){
            results.push(execCommand(commands[x], settings));
        }
    }
            ...
            exec('git commit --message="Build Master:' + new Date().toISOString() + '"',
                {cwd: dist }, function(error2, stdout2, stderr2){
                    if (error2) {
                        ...
                        exec('git push --set-upstream origin ' + process.env.MASTERPACK_BRANCH, {cwd: dist}, function(error3, stdout3, stderr3){
                            if (error3) {
