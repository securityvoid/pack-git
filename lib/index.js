...
function prepareDistFolder(){
    ...
    var commands =  [ 'git stash', 'git checkout ' + process.env.PACKGIT_BRANCH, 'git pull' ];
    ...
    initDist(dist).then(execCommands.bind(this,commands, settings)).catch(function(failed){
        ...
        if(error.message && error.message.indexOf("'" + process.env.PACKGIT_BRANCH + "' did not match any") >= 0 ){
            execCommands(['git checkout --orphan ' + process.env.PACKGIT_BRANCH], settings).then(function(){
...
function webPackIt(webPackLib){
    ...
    var compiler = webPackLib({
        ...
        try {
            //fs.unlinkSync(path.join(base, folder, process.env.PACKGIT_OUTPUT_LIBRARY));
        } catch (error){
...
function checkEnvVariables(){
    ...
    try{
        ...
        process.env.PACKGIT_IGNORED_MODULES = '["crypto", "openpgp"]';
    }
}
