...
if(argv['target'])
    process.env.MASTERPACK_SOURCE=argv['target'];

if(!process.env.MASTERPACK_SOURCE) {
    console.log("ENV variable DEPLOYMENT_SOURCE must be set or a value passed for --target")
    process.exit(1)
}

process.env.MASTERPACK_SOURCE=path.resolve(process.env.MASTERPACK_SOURCE);

var fs = require('fs');
if (!fs.existsSync(process.env.MASTERPACK_SOURCE)) {
